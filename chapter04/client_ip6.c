...
client_socket(const char *hostnm, const char *portnm)
{
    ...
    if (inet_pton(AF_INET6, hostnm, &addr) == 0) {
        /* ホスト名が名称としてホスト情報取得 */
        if ((host = gethostbyname2(hostnm, AF_INET6)) == NULL) {
            /* ホストが見つからない */
            (void) fprintf(stderr, "gethostbyname2():error\n");
            return (-1);
        }
        (void) memcpy(&addr, (struct in6_addr *) *host->h_addr_list, sizeof(struct in6_addr));
    }
